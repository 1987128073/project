...
class Aid(object):
    ...
    def get_liveid_task(self, days, id):
        '''
        :param days: 3,7,30
        :return:
        '''
        ...
        if response.get('result') and response.get('result').get('liveScene'):
            ...
            for index in range(response.get('liveScene')):
                liveid = response.get('liveIdList')[index]
                self.r.sadd('First_task_anchorId_liveId', '{}:{}'.format(id, liveid))
    ...
    def save_liveid(self, liveid, anchorId, livelist):

        res = self.db['LiveId'].find_one({'_id': str(liveid)})
        if not res:
            data = {
                '_id': str(liveid),
            }
            self.db['LiveId'].insert_one(data)
        res2 = self.db['Id_of_live_broadcast_of_anchor'].find_one({'_id': anchorId})
        if not res2:
            data = {
                '_id': str(anchorId),
                'livelist': livelist,
    ...
    def save_liveid_goods(self, liveid, result):
        ...
        res = self.db['LiveIdData'].find_one({'_id': liveid})
        if not res:
            data = {
                '_id': str(liveid),
                'livedata': result,
