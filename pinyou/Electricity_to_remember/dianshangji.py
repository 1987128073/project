...
class DianShangJi(object):
                ...
                for a in html_data:
                    if a.text[:10] == data.get('createTime')[:10]:
                        ...
                        try:
                            ...
                            break
                        self.insert_data(data, Monthly_payment, CommtentCount)
    ...
    def insert_data(self, data, Monthly_payment, CommtentCount):
        res = self.db['new_tb_anchor_goods'].find_one({'accountId': str(data.get('accountId')), 'itemId': str(data.get('itemId'))})
        if not res:
            data['Monthly_payment'] = Monthly_payment
            data['CommtentCount'] = CommtentCount
            self.db['new_tb_anchor_goods'].insert_one(data)
